(function () {
  ...
  function setupAboutCrossingTypography() {
    ...
    async function init() {
      ...
      if (isMobile) {
        // 모바일: 영문 두 줄 / 한글 두 줄 덩어리가 각각 통째로 반대 방향으로, PC보다
        // 훨씬 넓은 폭(50)으로 빠르게 스쳐 지나갑니다. 끝나는 지점(0)은 각 요소의
        // 원래 제자리라 교회소개가 화면 위쪽까지 올라왔을 때 화면 밖으로 잘리지
        // 않고, 시작 지점만 그만큼 반대쪽으로 당겨서(아래에서 올라오는 동안) 같은
        // 속도로 스쳐 지나가 보이게 합니다.
        scrollSlide(leftEl, -50, 0);
        scrollSlide(rightEl, 50, 0);
      } else {
