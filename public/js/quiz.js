...
(function () {
  ...
  function renderNameAndRead() {
    ...
    const readHtml = quiz.verses
      .map((v) => {
        const refHeading =
          v.reference && v.reference !== lastRef
            ? (() => {
                lastRef = v.reference;
                return `<p class="quiz-read-ref">${escapeHtml(v.reference)}</p>`;
              })()
            : '';
        return `${refHeading}<div class="quiz-read-verse">${v.verseLabel ? `<span class="num">${escapeHtml(v.verseLabel)}</span>` : ''}${escapeHtml(v.fullText)}</div>`;
      })
      .join('');
  ...
  function renderVerseStep() {
    ...
    mainEl.innerHTML = `
      <div class="quiz-card">
        ${nameFieldHtml}
        <p class="quiz-progress">${verseIndex + 1} / ${quiz.verses.length} 문제</p>
        <p class="quiz-verse-ref">${escapeHtml([verse.reference, verse.verseLabel ? verse.verseLabel + '절' : ''].filter(Boolean).join(' · '))}</p>
  ...
  function showRetryOptions(verse) {
    ...
    row.querySelector('.hint-btn').addEventListener('click', () => {
      ...
      const hintLabel = [verse.reference, verse.verseLabel ? verse.verseLabel + '절' : ''].filter(Boolean).join(' ');
      hintBox.textContent = `${hintLabel} 원문: ${verse.fullText}`;
